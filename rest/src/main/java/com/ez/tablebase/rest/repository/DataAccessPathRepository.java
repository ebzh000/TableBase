...
public interface DataAccessPathRepository extends JpaRepository<DataAccessPathEntity, String>
{
    ...
    @Query(value = "select category_id from categories where table_id = :tableId and category_id IN (select category_id from data_access_path where entry_id = :entryId and table_id = :tableId)", nativeQuery = true)
