...
app.get('/user/:id', function (req, res) {
  ...
  if (!userId) {
    return res.status(400).send({error: true, message: 'Please enter a valid user id'})
  }

  db.query("SELECT * FROM users WHERE user_id = ?", userId, function (error, results, fields) {
